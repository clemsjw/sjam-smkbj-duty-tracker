...
async function initDB() {
  ...
  await db.execute(`CREATE TABLE IF NOT EXISTS slides (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
...
}, async (nombor_daftar, password, done) => {
...
passport.deserializeUser(async (nombor_daftar, done) => {
  const users = await query('SELECT * FROM users WHERE nombor_daftar = ?', [nombor_daftar]);
...
app.post('/update-status/:id', checkAuth, async (req, res) => {
  ...
  await run('UPDATE duties SET status = ?, reason = ? WHERE id = ?', [status, reason || null, req.params.id]);
...
app.post('/notifications/read', checkAuth, async (req, res) => {
  await run('UPDATE notifications SET read = 1 WHERE nombor_daftar = ?', [req.user.nombor_daftar]);
  res.json({ success: true });
});

app.post('/notifications/clear', checkAuth, async (req, res) => {
  await run('DELETE FROM notifications WHERE nombor_daftar = ?', [req.user.nombor_daftar]);
  res.json({ success: true });
});
...
app.get('/admin', checkAuth, async (req, res) => {
  ...
  const year = req.query.year || 'all';
  const allRecords = await query('SELECT d.id, d.nombor_daftar, d.event_name, d.start_time, d.end_time, d.hours, d.status, d.reason, d.created_at, u.full_name, u.class FROM duties d JOIN users u ON d.nombor_daftar = u.nombor_daftar ORDER BY d.created_at DESC');
...
app.get('/admin/students', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.redirect('/dashboard');
  const students = await query('SELECT * FROM users ORDER BY class DESC, full_name ASC');
...
app.get('/admin/slides', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.redirect('/dashboard');
  const slides = await query('SELECT * FROM slides ORDER BY id');
  res.render('admin-slides', { user: req.user, slides: slides });
});

app.post('/api/slides/add', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Unauthorized' });
  const { image_url, title } = req.body;
  await run('INSERT INTO slides (image_url, title) VALUES (?, ?)', [image_url, title || '']);
  res.json({ success: true });
});

app.post('/api/slides/delete/:id', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Unauthorized' });
...
app.get('/admin/student/:nd/export', checkAuth, async (req, res) => {
  ...
  const allDuties = await query('SELECT * FROM duties WHERE nombor_daftar = ? ORDER BY created_at ASC', [req.params.nd]);
  ...
  duties.forEach((d, i) => {
    ...
    csv += `${i + 1},${d.event_name},${(d.hours || 0).toFixed(1)},${String(date.getDate()).padStart(2,'0')}/${String(date.getMonth() + 1).padStart(2,'0')}/${date.getFullYear()}\n`;
  });
...
app.get('/admin/db-editor', checkAuth, async (req, res) => {
  ...
  const tables = {
    users: await query('SELECT * FROM users ORDER BY class DESC, full_name ASC'),
    duties: await query('SELECT * FROM duties ORDER BY created_at DESC'),
    notifications: await query('SELECT * FROM notifications ORDER BY created_at DESC'),
    slides: await query('SELECT * FROM slides')
  };
  res.render('db-editor', { tables: tables });
});

app.post('/api/db/update', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Unauthorized' });
  ...
  await run(`UPDATE ${table} SET ${sets} WHERE ${idCol} = ?`, [...values, id]);
  res.json({ success: true });
});

app.post('/api/db/delete', checkAuth, async (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Unauthorized' });
  const { table, id } = req.body;
  const idCol = table === 'users' ? 'nombor_daftar' : 'id';
  await run(`DELETE FROM ${table} WHERE ${idCol} = ?`, [id]);
  res.json({ success: true });
});
