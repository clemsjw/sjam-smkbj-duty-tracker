...
const db = new Database('./school_duty.db');
...
passport.use(new LocalStrategy({
  ...
  const user = db.prepare('SELECT * FROM users WHERE ic_number = ?').get(ic_number);
...
app.get('/dashboard', checkAuth, (req, res) => {
  const duties = db.prepare('SELECT * FROM duties WHERE user_ic = ? ORDER BY created_at DESC').all(req.user.ic_number);
...
app.post('/add-duty', checkAuth, (req, res) => {
  ...
  db.prepare('INSERT INTO duties (user_ic, event_name, start_time, end_time, hours) VALUES (?, ?, ?, ?, ?)').run(req.user.ic_number, event_name, start_time, end_time, hours);
  res.redirect('/dashboard');
});

app.post('/update-status/:id', checkAuth, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Unauthorized' });
  db.prepare('UPDATE duties SET status = ? WHERE id = ?').run(req.body.status, req.params.id);
...
app.get('/admin/students', checkAuth, (req, res) => {
  if (req.user.role !== 'admin') return res.redirect('/dashboard');
  const students = db.prepare('SELECT * FROM users WHERE role = ? ORDER BY full_name').all('student');
  res.render('admin-students', { user: req.user, students: students });
});

app.get('/admin/student/:ic', checkAuth, (req, res) => {
  if (req.user.role !== 'admin') return res.redirect('/dashboard');
  const student = db.prepare('SELECT * FROM users WHERE ic_number = ?').get(req.params.ic);
  if (!student) return res.redirect('/admin/students');
  const duties = db.prepare('SELECT * FROM duties WHERE user_ic = ? ORDER BY created_at DESC').all(req.params.ic);
